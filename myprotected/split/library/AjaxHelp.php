...
class ajaxHelp
{
	...
	// Send mail
	public function wp_send_letter($to_mail,$from,$subject_mail,$message_mail,$system="ItFurLine")
	{
		try
		{
			$date = date("d.m.y H:i");
			...
			$message_attach = "<br> <p>Best regards, <a href=\"http://www.itfurline.com\">ItFurLine</a></p>".$message_date;
...
public function send_letter($to_mail,$from,$subject_mail,$message_mail,$system="ItFurLine")
	{
		try
		{
			$date = date("d.m.y H:i");
	...
	public function print_adding_products_list_table($products,$orderId,$type=false)
	{
		...
		$str = "";
				...
				foreach($products as $prod)
				{
					...
					$query = "SELECT cat_id 
						FROM [pre]shop_cat_prod_ref 
						WHERE `prod_id`='$pid' 
						LIMIT 1";
					$prodcat = $this->rs($query);
					
					$cat_id = $prodcat['0']['cat_id'];
					...
					$query = "SELECT * 
						...
						AND `id`='$cat_id' 
						LIMIT 1";
					$category = $this->rs($query);
					
					$cat_chars_group_id = $category['0']['specs_group_id'];
					...
					foreach($dinamic_chars as $i => $d_item)
					{
						$d_id = $d_item['id'];
						...
						$query = "SELECT M.id, M.value, M.value2, M.filter, M.price_dif,
							 ...
							 WHERE M.char_id='".$d_id."' AND M.prod_id=$pid
							 ORDER BY M.price_dif
							 LIMIT 1000";
	
						$dinamic_chars[$i]['values'] = $this->rs($query);
					}

					//echo "<pre>"; print_r($dinamic_chars); echo "</pre>"; exit();
					...
					$str .= "
						...
						$str .= "<td style='padding-left:10px;'>".$prod['price']."</td>";
						...
						if($dinamic_chars)
						{
							...
							foreach($dinamic_chars as $dc)
							{
								$str .= "<select class='styled-select din_add_char_$pid' name='dinamic_char[".$dc['id']."]' charid='".$dc['id']."' >
									<option value='0'>(".$dc['name'].")</option>";
									foreach($dc['values'] as $value)
									{
										$str .= "<option value='".$value['id']."'>".$value['value2']."</option>";
									}
								$str .= "</select>";
							}
							$str .= "</td>";
