...
class BasicHelp extends BasicPrinter
{
		...
		public function getLandingHeader($params=array())
		{
			...
			{
				$result .= "<button class='rzh r-z-h-s-create' type='button' onclick=\"export_products_to_excel();\" title='Export to Excel'>Export</button>";
			}
				...
				switch($params['type'])
				{
					...
					case 'shopManage':
					{
						...
						$result .= "
								...
								<button class='rzh r-z-h-s-create' type='button' onclick=\"$('#check_import_file').click();\" title='Import from Excel'>Импорт обновлений</button>
				...
				switch($params['type'])
				{
					...
					case 'articles-manager':
					{
						$result .=	"
        	    	  	...
        	    	  	<button class='rzh r-z-h-s-create' type='button' onclick=\"open_new_modal('confirmSendArtToSubscribers',{article_id:$item_id});\" title='Отправить новость подписчикам'> &nbsp; Разослать подписчикам &nbsp;<span></span></button>
			...
			$result .= "
				<div class='head-app' id='head-app-1'>
					...
					else
					{
            			...
            			<button class='rzh r-z-h-s-new' type='button' title='Создать и скопировать' onclick=\"submit_create_form(4);\"></button>
						";
					}
					$result .= "
					</div>
		...
		public function getCardViewTable($params=array())
		{
			...
			if($tabsManager)
			{
				...
				$result .= "<li class='".(!$lpx ? "active" : "")."' onclick=\"playSound();\">
								<button type='button' onclick=\"loadPage('$_parent','$_alias',$_id,$_item_id,'cardView',{lpx:''});\">RUSSIAN</button>
							</li>";
				...
				switch($tmp['type'])
				{
					...
					case 'images':
					{
						...
						foreach($params['cardItem'][$tmp['field']] as $currImg)
						{
							...
							if($tmp['params']['path']=="/split/files/shop/products/")
							{
							...
							<a class='theater' href='".$filePath."' title='Фото товара' rel='group'>
		...
		public function getCardEditForm($params=array())
		{
			...
			if($tabsManager)
			{
				...
				$result .= "<li class='".(!$lpx ? "active" : "")."' onclick=\"playSound();\">
								<button type='button' onclick=\"loadPage('$_parent','$_alias',$_id,$_item_id,'cardEdit',{lpx:''});\">RUSSIAN</button>
							</li>";

				foreach($langs as $lang){
					$result .= "<li class='".($lpx==$lang['alias'] ? "active" : "")."' onclick=\"playSound();\">
									<button type='button' onclick=\"loadPage('$_parent','$_alias',$_id,$_item_id,'cardEdit',{lpx:'".$lang['alias']."'});\">".strtoupper($lang['name'])."</button>
								</li>";
				}
				$result .= "</ul>";
				$result .= "<div class='clear'></div>";
			...
			foreach($params['cardTmp'] as $fieldHeader => $tmp)
			{
				...
				switch($tmp['type'])
				{
					...
					case 'hidden':
					{
						$value = (isset($tmp['params']['arr_field']) ? $item[$tmp['params']['field']][$tmp['params']['arr_field']] : $item[$tmp['field']]);
					...
					case 'input':
					{
						...
						$result .= $this->print_input($fieldHeader, $tmp['field'], $tmp['params']['hold'], $inputValue, $tmp['params']['size'], $tmp['params']['onchange'],$inputType,$inputDisabled);
						break;
					}
					case 'autocomplete':
					{
					...
					case 'color':
					{
						$result .= $this->print_input($fieldHeader, $tmp['field'], $tmp['params']['hold'], $item[$tmp['field']], $tmp['params']['size'], $tmp['params']['onchange'],"color");
						break;
					}
					case 'area':
					{
					...
					case 'image_mult':
					{
						...
						$result .= $this->print_image_mult($fieldHeader, $tmp['field'], $item[$tmp['field']], $tmp['params']['path'], $tmp['params']['appTable'], $tmp['params']['id'], $tmp['params']['field'], $method);
						break;
					}
					...
					case 'projectChars':
					{
						$result .= $this->print_projectChars($tmp['field'],$tmp['params']['chars']);
						break;
					}
					case 'projectDinamicChars':
					{
						$result .= $this->print_projectDinamicChars($tmp['field'],$tmp['params']['chars']);
						break;
					}
					case 'cartProducts':
					{
						$type = (isset($tmp['params']['type']) ? $tmp['params']['type'] : false);
						$result .= $this->print_cart_products($item_id,$tmp['params']['items'],$tmp['params']['node_list'],$tmp['params']['delivery_price'],$type);
						break;
					}
					case 'usersExtraFields':
					{
