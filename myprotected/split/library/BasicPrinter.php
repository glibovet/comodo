...
class BasicPrinter
{
			...
			<div class='zen-form-item'>
				...
				<div class='zif-wrap'>
                	<input	id='save-$name' class='my-field' type='$input_type' placeholder='$hold' onchange=\"$onchange\" onkeyup=\"$onchange\" 
                    		value='".( str_replace("'","&rsquo;",$value) )."' name='$name' size='$size' $disabledTxt />
                </div>
            </div>
		...
		public function print_image_mono($header, $field, $value, $path, $appTable ,$id)
		{
            ...
            if(file_exists("../../../../..".$path.$value) && trim($value) != "" || true)
			{
						...
						if($file_ext=='mp4' || $file_ext=='3gp' || $file_ext=='webm')
						{
							$is_video = true;
						}
				...
				<div class='item' id='media-item-$field'>
					...
					if($is_video)
					{
						...
						<video width='auto' height='100%' controls>
						  <source src='".$path.$value."' type='video/$file_ext'>
						  Ваш браузер не поддерживает HTML5 видео в $file_ext.
						</video>
						";
					}else{
						$result .= "
            ...
            if(file_exists("../../../../..".$path.$val_item[$mas_field]) && trim($val_item[$mas_field]) != "" || true)
			{
				...
				<div class='item' id='media-item-".$val_item['id']."'>
					...
					<div class='file-manager-wrap'>
						...
						<input type='text' placegolder='FILE ALT' value=\"".(isset($val_item['title']) ? $val_item['title'] : "ItFurLine file")."\" onchange=\"save_new_file_alt(".$val_item['id'].",$(this).val(),'$field')\" />
						<label>Позиция</label>
			...
			<br>
				...
				if($has_chars)
				{
					foreach($chars as $char)
					{
                		...
                		<tr>
                	        <td>
                	        	<input id='char-".$char['char_id']."' chartype='static' attrID='".$char['char_id']."' class='my-field autocomplete' type='text' placeholder='".$char['title']."' value='".$char['value']."' name='".$field."[".$char['char_id']."]' size='25' maxlength='100'>
							</td>
                	        <td>
                	        	<input id='char3-".$char['char_id']."' class='my-field' type='text' placeholder='".$char['title']."' value='".$char['ua_value']."' name='".$field."3[".$char['char_id']."]' size='25' maxlength='100'>
							</td>
                	        <td>
                	        	<input id='char5-".$char['char_id']."' class='my-field' type='text' placeholder='".$char['title']."' value='".$char['en_value']."' name='".$field."5[".$char['char_id']."]' size='25' maxlength='100'>
							</td>
						</tr>
						<tr>
                	    	<td class='char_bord' colspan='3'></td>
						</tr>
						";
					}
				}else
	...
	public function print_shopProductDinamicChars($header,$field,$chars,$has_chars=0)
	{
		...
		?>
			...
			<br>
        	        <p title='Группа характеристик'>Группа свойств: <b><?php echo $header ?></b>
				</p>
            <br>
		...
		if($has_chars)
		{
			foreach($chars as $char)
			{
				$_id = $char['id'];
				$_name = $char['name'];
				$_measure = trim($char['measure']);
				?>
                    <tr id="dinamicChar-<?php echo $_id ?>" >
                        <td><?php echo $_name."".($_measure!="" ? ", ".$_measure : "") ?></td>
                        <td class="d_chars_values">
                        	<table>
                            	<thead>
                                	<tr>
                                    	<th >RU</th>
                                ...
                                foreach($char['values'] as $item)
								{
									...
									<tr id="dinamic_char_value_<?php echo $refID ?>">
                                    	<td><input attrID="<?php echo $refID ?>" chartype="dinamyc" id="d_char_val_value<?php echo $refID ?>" class="my-field autocomplete" size="15" type="text" value="<?php echo $item['value'] ?>" placeholder="RU" onChange="save_dinamic_char_value_text(<?php echo $refID ?>,<?php echo $_id ?>,'value',$(this).val(),$(this).parent().parent().find('#d_char_val_value<?php echo $refID ?>').val())" /></td>                                        
                                        <td><input id="d_char_ua_val_ua_value<?php echo $refID ?>" class="my-field" size="15" type="text" value="<?php echo $item['ua_value'] ?>" placeholder="UA" onChange="save_dinamic_char_value_text(<?php echo $refID ?>,<?php echo $_id ?>,'ua_value',$(this).val(),$(this).parent().parent().find('#d_char_val_value<?php echo $refID ?>').val())" /></td>
                                        <td><input id="d_char_en_val_en_value<?php echo $refID ?>" class="my-field" size="15" type="text" value="<?php echo $item['en_value'] ?>" placeholder="EN" onChange="save_dinamic_char_value_text(<?php echo $refID ?>,<?php echo $_id ?>,'en_value',$(this).val(),$(this).parent().parent().find('#d_char_val_value<?php echo $refID ?>').val())" /></td>
                                        
                                        <!--<td><input id="d_char_val_price_<?php echo $refID ?>" class="my-field" id="25" type="number" value="<?php echo $item['price_dif'] ?>" placeholder="Введите доп. цену" onChange="save_dinamic_char_value_price(<?php echo $refID ?>,$(this).val())" /></td>-->
                        ...
                        <td>
                            <button class="r-z-h-s-create nssBut fLeft" type="button" onclick="add_new_dinamic_char(<?php echo $_id ?>);">Добавить &nbsp;&nbsp;&nbsp;<span>+</span></button>
                        </td>
                    </tr>
				<?php
			}
		...
		public function print_projectChars($field,$chars)
		{
			$result = "";
			$result .= "
			<div id='wp-form-extra-fields-wrap'>            	
			<br>
				...
				foreach($chars as $char)
				{
					...
					<tr>
						<td>
							<input id='char-".$char['char_id']."' attrID='".$char['char_id']."' chartype='static' class='my-field autocomplete' type='text' placeholder='".$char['title']."' value='".$char['value']."' name='".$field."[".$char['char_id']."]' size='25' maxlength='100'>
						</td>
						<td>
							<input id='char3-".$char['char_id']."' class='my-field' type='text' placeholder='".$char['title']."' value='".$char['ua_value']."' name='".$field."3[".$char['char_id']."]' size='25' maxlength='100'>
						</td>
		...
		// Project Dinamic Chars table
	
		public function print_projectDinamicChars($field,$chars)
		{
			$result = "";
					...
					foreach($chars as $char)
					{
						...
						?>
							<tr id="dinamicChar-<?php echo $_id ?>" >
								...
								<td class="d_chars_values">
									<table>
										...
										foreach($char['values'] as $item)
										{
											...
											?>
											<tr id="dinamic_char_value_<?php echo $refID ?>">
												<td><input attrID="<?php echo $refID ?>" chartype="dinamyc" id="d_char_val_value<?php echo $refID ?>" class="my-field autocomplete" size="15" type="text" value="<?php echo $item['value'] ?>" placeholder="RU" onChange="save_proj_dinamic_char_value_text(<?php echo $refID ?>,<?php echo $_id ?>,'value',$(this).val(),$(this).parent().parent().find('#d_char_val_value<?php echo $refID ?>').val())" /></td>                                        
												<td><input id="d_char_ua_val_ua_value<?php echo $refID ?>" class="my-field" size="15" type="text" value="<?php echo $item['ua_value'] ?>" placeholder="UA" onChange="save_proj_dinamic_char_value_text(<?php echo $refID ?>,<?php echo $_id ?>,'ua_value',$(this).val(),$(this).parent().parent().find('#d_char_val_value<?php echo $refID ?>').val())" /></td>
												<td><input id="d_char_en_val_en_value<?php echo $refID ?>" class="my-field" size="15" type="text" value="<?php echo $item['en_value'] ?>" placeholder="EN" onChange="save_proj_dinamic_char_value_text(<?php echo $refID ?>,<?php echo $_id ?>,'en_value',$(this).val(),$(this).parent().parent().find('#d_char_val_value<?php echo $refID ?>').val())" /></td>
								...
								<td>
									<button class="r-z-h-s-create nssBut fLeft" type="button" onclick="add_new_proj_dinamic_char(<?php echo $_id ?>);">Добавить &nbsp;&nbsp;&nbsp;<span>+</span></button>
								</td>
							</tr>
						<?php
                        }
		...
		public function print_cart_products($orderId,$products,$nodeList=array(),$deliverySum=0,$type=false)
		{
			$str = "";
			...
			$str .=  "
					...
					$str .= "<th class='main-t-th'>Размер</th>";
			...
			foreach($products as $pi => $product)
			{
				...
				if(isset($product['id']) && $product['id'])
				{
					...
					$prodMassive = $this->rs("SELECT id,name,sku,code,price FROM [pre]shop_products WHERE id=".$product['id']." LIMIT 1");
				}else
				{
					continue;
				}
				...
				$str .= "
						...
						$str .= "<td style='padding-left:10px;'><a target='_blank' href='http://itfurline.com/catalog/product/".$product['alias']."/'>".$curr_name."<a>";
			...
			$str .= "
					<table class='intotal w100'>
						<tr>
							...
							<th>Всего:		<span id='createOrderTotalSum'>".($orderSum+$deliverySum)."</span> грн.</th>
