		...
		if($defVal)
		{
			switch($field_to_update)
			{
				case 'value':
				{
					...
					{
						$query = "INSERT INTO [pre]shop_chars_values (`char_id`,`value`) 
				...
				case 'ua_value':
				{
					...
					$findValQuery = "SELECT id FROM [pre]shop_chars_values WHERE `char_id`='$char_id' AND `value`='$defVal' LIMIT 1";
					$findVal = $ah->rs($findValQuery);
					...
					if($findVal)
					{
						...
						$query = "UPDATE [pre]shop_chars_values SET `ua_value`='$ua_value' WHERE `id`='$charValId' LIMIT 1";
						$ah->rs($query);
					}else
					{
						$query = "INSERT INTO [pre]shop_chars_values (`char_id`,`value`,`ua_value`) 
				...
				case 'en_value':
				{
					...
					if($findVal)
					{
						...
						$query = "UPDATE [pre]shop_chars_values SET `en_value`='$en_value' WHERE `id`='$charValId' LIMIT 1";
						$ah->rs($query);
					}else
					{
						$query = "INSERT INTO [pre]shop_chars_values (`char_id`,`value`,`en_value`) 
